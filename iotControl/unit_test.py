...
class TestPowerSession():
    ...
    def create_power_session(self):
        sql_query = db_queries.create_power_session(self.device_switch_id, self.trigger_on_time)
        db.run_qry(sql_query)

    def complete_power_session(self):
        sql_query = db_queries.complete_power_session(self.device_switch_id, self.trigger_off_time)
        db.run_qry(sql_query)
